<template>
  <div>
        ...
        <form>
            ...
            <div class="row">
                <div class="center">
                    <button class="button-primary" v-on:click.prevent="updateGraveYeard()">Zmień</button>
                </div>
            </div>
...
export default {
    ...
    created() {
        this.getGraveYeard(this.cmID)
    },
    methods: {
        updateGraveYeard: function() {
            graveyeards.editCm(this.cmData, this.cmID)
            document.forms[0].reset()
            this.$router.push({ path: `/` })
        },
        getGraveYeard: function(id) {
            let self = this;
            graveyeards.viewOne(id, function(res) {
                if (res) {
                    self.cmArr=res
                    self.cmData.cName = self.cmArr.thecm.cName
                    self.cmData.cmFullName = self.cmArr.thecm.cmFullName
                    self.cmData.wejscie = self.cmArr.thecm.wejscie
                    self.cmData.rzad = self.cmArr.thecm.rzad
                    self.cmData.grob = self.cmArr.thecm.grob
